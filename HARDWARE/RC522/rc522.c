...
void rc522port_Init(void)	
{
  ...
  SPI_InitTypeDef  SPI_InitStructure;
	...
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;  //PA13/14/15复用推挽输出 
...
char PcdRequest(u8   req_code,u8 *pTagType)
{
	...
	ClearBitMask(Status2Reg,0x08);//清RC522寄存器位 第一个参数都是寄存器  包含接收器和发送器的状态标志
	WriteRawRC(BitFramingReg,0x07);//写RC632寄存器 面向位的帧的调节
	SetBitMask(TxControlReg,0x03);//置RC522寄存器位  控制天线驱动器管脚TX1和TX2的逻辑特性
 
	ucComMF522Buf[0] = req_code;//main里面的参数是0x52 查看全部的卡

	status = PcdComMF522(PCD_TRANSCEIVE,ucComMF522Buf,1,ucComMF522Buf,&unLen);//RC522和ISO14443卡通讯
...
//返    回: 成功返回MI_OK
///////////////////////////////////////////////////////////////////// 
char PcdRead(u8   addr,u8 *p )//块地址是一个数组，一共是64位
{
    char   status;
    u8   unLen;
...
{
   if (type == 'A')                     //ISO14443_A   串口输进去A，就是自动寻卡模式？
   { 
       ClearBitMask(Status2Reg,0x08);
       WriteRawRC(ModeReg,0x3D);//3F    写RC632寄存器
